    ...
    return response.endpoints.map((ep: any) => ({
      ...
      audit: {
        ...
        security: {
          ...
          encryption: ep.audit?.encryption_enabled ?? null,
          antivirus: ep.audit?.antivirus_installed ?? ep.antivirus_active ?? false,
          secureBoot: ep.audit?.secure_boot_enabled ?? null,
          tpm: ep.audit?.tpm_present ?? null,
          ssh: ep.audit?.ssh_enabled ?? null,
          auditd: ep.audit?.auditd_enabled ?? null,
          passwordlessSudo: ep.audit?.passwordless_sudo ?? null,
          sip: ep.audit?.sip_enabled ?? null,
          gatekeeper: ep.audit?.gatekeeper_enabled ?? null,
        },
